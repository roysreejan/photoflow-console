...
const Verify = () => {
  ...
  const handleResendOtp = async () => {
    ...
    const result = await handleAuthRequest(resendOtp, setIsLoading);
  ...
  return (
    <div className="h-screen flex items-center flex-col justify-center">
      ...
      <div className="flex items-center mt-4 space-x-2">
        <h1 className="text-sm sm:text-lg font font-medium text-gray-700">
          Didn&apos;t Get the otp code?{" "}
        </h1>
        <button
          onClick={handleResendOtp}
          className="test-sm sm:text-lg font-medium text-blue-900 underline cursor-pointer"
        >
          Resend Code
        </button>
      </div>
      <LoadingButton
        ...
        isLoading={isLoading}
        size={"lg"}
        className="mt-6 w-52 bg-black text-white hover:bg-gray-800"
      >
        Verify
      </LoadingButton>
    </div>
